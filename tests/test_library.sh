...
LIBRARY_TESTS="block block_descriptor block_range_descriptor block_tree error io_handle notify store store_block store_descriptor";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./vshadow_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libvshadow" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
