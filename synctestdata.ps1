...
If (-Not (Test-Path "${TestsInputDirectory}\.vshadowinfo"))
{
	New-Item -Name "${TestsInputDirectory}\.vshadowinfo" -ItemType "directory" | Out-Null
	Write-Output "-a" | Out-File -Encoding ascii -FilePath "${TestsInputDirectory}\.vshadowinfo\options"
}
