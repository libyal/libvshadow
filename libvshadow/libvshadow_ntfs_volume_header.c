...
{
	static char *function = "libvshadow_ntfs_volume_header_initialize";
	...
	*ntfs_volume_header = memory_allocate_structure(
	                  libvshadow_ntfs_volume_header_t );
	...
	if( memory_set(
	     *ntfs_volume_header,
	     0,
	     sizeof( libvshadow_ntfs_volume_header_t ) ) == NULL )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_MEMORY,
...
int libvshadow_ntfs_volume_header_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "libvshadow_ntfs_volume_header_free";
...
int libvshadow_ntfs_volume_header_read_data(
     ...
     libcerror_error_t **error )
{
	static char *function                    = "libvshadow_ntfs_volume_header_read_data";
	...
	if( ( data_size < sizeof( vshadow_ntfs_volume_header_t ) )
...
#endif
	if( memory_compare(
	     ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature,
	     "NTFS    ",
	     8 ) != 0 )
	{
		libcerror_error_set(
	...
	byte_stream_copy_to_uint16_little_endian(
	 ( (vshadow_ntfs_volume_header_t *) data )->bytes_per_sector,
	 ntfs_volume_header->bytes_per_sector );

	byte_stream_copy_to_uint64_little_endian(
	 ( (vshadow_ntfs_volume_header_t *) data )->total_number_of_sectors,
	 ntfs_volume_header->volume_size );

	byte_stream_copy_to_uint64_little_endian(
	 ( (vshadow_ntfs_volume_header_t *) data )->mft_cluster_block_number,
	 mft_cluster_block_number );

	byte_stream_copy_to_uint64_little_endian(
	 ( (vshadow_ntfs_volume_header_t *) data )->mirror_mft_cluster_block_number,
	 mirror_mft_cluster_block_number );

	byte_stream_copy_to_uint32_little_endian(
	 ( (vshadow_ntfs_volume_header_t *) data )->mft_entry_size,
	 mft_entry_size );

	byte_stream_copy_to_uint32_little_endian(
	 ( (vshadow_ntfs_volume_header_t *) data )->index_entry_size,
	 index_entry_size );
	...
	if( libcnotify_verbose != 0 )
	{
		libcnotify_printf(
		 ...
		 function,
		 ( (vshadow_ntfs_volume_header_t *) data )->boot_entry_point[ 0 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->boot_entry_point[ 1 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->boot_entry_point[ 2 ] );

		libcnotify_printf(
		 "%s: file system signature\t\t: %c%c%c%c%c%c%c%c\n",
		 function,
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 0 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 1 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 2 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 3 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 4 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 5 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 6 ],
		 ( (vshadow_ntfs_volume_header_t *) data )->file_system_signature[ 7 ] );
		...
		libcnotify_printf(
		 ...
		 function,
		 ( (vshadow_ntfs_volume_header_t *) data )->sectors_per_cluster_block );

		libcnotify_printf(
		 "%s: unknown1\n",
		 function );
		libcnotify_print_data(
		 ( (vshadow_ntfs_volume_header_t *) data )->unknown1,
		 7,
		 0 );
		...
		libcnotify_printf(
		 ...
		 function,
		 ( (vshadow_ntfs_volume_header_t *) data )->media_descriptor );

		byte_stream_copy_to_uint16_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->unknown2,
		 value_16bit );
		libcnotify_printf(
		 "%s: unknown2\t\t\t: %" PRIu16 "\n",
		 function,
		...
		byte_stream_copy_to_uint16_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->sectors_per_track,
		 value_16bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint16_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->number_of_heads,
		 value_16bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint32_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->number_of_hidden_sectors,
		 value_32bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint32_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->unknown3,
		 value_32bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint32_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->unknown4,
		 value_32bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint64_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->volume_serial_number,
		 value_64bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint32_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->checksum,
		 value_32bit );
		libcnotify_printf(
		...
		libcnotify_printf(
		 ...
		 function );
		libcnotify_print_data(
		 ( (vshadow_ntfs_volume_header_t *) data )->bootcode,
		 426,
		 0 );

		byte_stream_copy_to_uint16_little_endian(
		 ( (vshadow_ntfs_volume_header_t *) data )->sector_signature,
		 value_16bit );
		libcnotify_printf(
	...
	cluster_block_size = ( (vshadow_ntfs_volume_header_t *) data )->sectors_per_cluster_block;
...
int libvshadow_ntfs_volume_header_read_file_io_handle(
     ...
     libcerror_error_t **error )
{
	uint8_t ntfs_volume_header_data[ sizeof( vshadow_ntfs_volume_header_t ) ];

	static char *function = "libvshadow_ntfs_volume_header_read_file_io_handle";
	...
	{
		libcnotify_printf(
		 "%s: reading NTFS volume header at offset: %" PRIi64 " (0x%08" PRIx64 ")\n",
		 function,
		 file_offset,
		 file_offset );
	}
	...
	read_count = libbfio_handle_read_buffer_at_offset(
	              file_io_handle,
	              (uint8_t *) &ntfs_volume_header_data,
	              sizeof( vshadow_ntfs_volume_header_t ),
	              file_offset,
	              error );
	...
	if( read_count != (ssize_t) sizeof( vshadow_ntfs_volume_header_t ) )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to read NTFS volume header data at offset: %" PRIi64 " (0x%08" PRIx64 ").",
		 function,
		 file_offset,
		 file_offset );
	...
	if( libvshadow_ntfs_volume_header_read_data(
	     ntfs_volume_header,
	     (uint8_t *) &ntfs_volume_header_data,
	     sizeof( vshadow_ntfs_volume_header_t ),
	     error ) != 1 )
