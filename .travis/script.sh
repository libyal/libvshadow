...
then
	docker run -t -v "${PWD}:/libvshadow" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libvshadow && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libvshadow;
