      ...
      libcerror_error_t *error )
{
	...
	char extended_format_string[ PYVSHADOW_ERROR_FORMAT_STRING_SIZE ];
	...
	if( function == NULL )
	{
		function = "pyvshadow_error_raise";
	}
	...
	if( error != NULL )
	{
		...
		if( format_string_length < ( PYVSHADOW_ERROR_FORMAT_STRING_SIZE - 4 ) )
		{
			     ...
			     PYVSHADOW_ERROR_STRING_SIZE ) != -1 )
			{
				     ...
				     format_string_length ) != NULL )
				{
					...
					for( error_string_index = 0;
					     error_string_index < PYVSHADOW_ERROR_STRING_SIZE;
					     error_string_index++ )
					{
