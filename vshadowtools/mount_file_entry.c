     ...
     libcerror_error_t **error )
{
	...
	if( file_entry->vshadow_store != NULL )
	{
     ...
     libcerror_error_t **error )
{
	...
	*number_of_sub_file_entries = number_of_stores;
         ...
         libcerror_error_t **error )
{
	...
	if( read_count < 0 )
	{
		libcerror_error_set(
		 ...
		 "%s: unable to read buffer at offset: %" PRIi64 " (0x%08" PRIx64 ") from store.",
		 function,
		 offset,
		 offset );
	...
	else
	{
		     ...
		     error ) != 1 )
		{
			libcerror_error_set(
			 ...
			 LIBCERROR_ERROR_DOMAIN_RUNTIME,
			 LIBCERROR_RUNTIME_ERROR_GET_FAILED,
			 "%s: unable to retrieve size from store.",
			 function );
